...
app.get('/api/support/conversations/:id', requireAuth, async (req, res) => {
  try {
    const pool = getDbPool();
    if (!pool) return res.status(400).json({ error: 'MySQL is not configured' });
...
app.delete('/api/support/conversations/:id', requireAuth, async (req, res) => {
  try {
    const pool = getDbPool();
    if (!pool) return res.status(400).json({ error: 'MySQL is not configured' });
    ...
    const isStaff = ['admin', 'moderator'].includes(role);
    const [crows] = await pool.query(`SELECT id, pro_user_id AS proUserId FROM support_conversations WHERE id = ? LIMIT 1`, [
      convId,
    ]);
    if (!crows?.length) return res.status(404).json({ error: 'Conversation not found' });
    const conv = crows[0];
...
app.post('/api/support/conversations/:id/typing', requireAuth, async (req, res) => {
  try {
    const pool = getDbPool();
    if (!pool) return res.status(400).json({ error: 'MySQL is not configured' });
...
app.post('/api/support/conversations/:id/messages', requireAuth, async (req, res) => {
  try {
    const pool = getDbPool();
    if (!pool) return res.status(400).json({ error: 'MySQL is not configured' });
...
app.post('/api/support/conversations/:id/seen', requireAuth, async (req, res) => {
  try {
    const pool = getDbPool();
    if (!pool) return res.status(400).json({ error: 'MySQL is not configured' });
    ...
    const [rows] = await pool.query(
      `
        ...
        WHERE ${isStaff ? '1=1' : 'c.pro_user_id = ?'}
          ...
          AND ${isStaff ? "su.role NOT IN ('admin','moderator')" : "su.role IN ('admin','moderator')"}
      `,
